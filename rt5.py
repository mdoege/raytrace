...
allL = [vec3(5, 5, -10), vec3(-3, 5, -1)]        # Point light positions
...
def raytrace(O, D, scene, bounce = 0):
    ...
    for (s, d) in zip(scene, distances):
        ...
        if np.any(hit):
            ...
            Oc = O.extract(hit)
            Dc = D.extract(hit)
            for Lind in range(len(allL)):
                cc = s.light(Oc, Dc, dc, scene, bounce, allL, Lind)
                color += cc.place(hit)
    return color
...
class Sphere:
    ...
    def light(self, O, D, d, scene, bounce, allL, Lind):
        L = allL[Lind]
        M = (O + D * d)                         # intersection point
        bfac = .01      # bump factor
        T = M - self.c
        dN = vec3(bfac * np.sin(10 * T.x), bfac * np.sin(10 * T.y), bfac * np.sin(10 * T.z))
        N = (M - self.c) * (1. / self.r) + dN       # normal
        N = N.norm()
        ...
        color += self.diffusecolor(M) * (lv + .1) * seelight
        color += self.diffusecolor(M) * .1 * (1 - seelight)

        # Reflection
        if bounce < 2 and Lind == 0:
...
Image.merge("RGB", rgb).save("rt5.png")
